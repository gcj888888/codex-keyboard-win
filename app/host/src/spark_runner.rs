...
pub const SPARK_MODEL: &str = "qwen3-max";
...
// codex 以 --ignore-user-config 运行时不加载 config.toml，必须在命令行内联 provider 定义
const SPARK_PROVIDER_CONFIG: &[&str] = &[
    "--config",
    "model_provider=\"qwen\"",
    "--config",
    "model_providers.qwen.name=\"Qwen (DashScope)\"",
    "--config",
    "model_providers.qwen.base_url=\"https://dashscope.aliyuncs.com/compatible-mode/v1\"",
    "--config",
...
impl Default for SparkRunnerConfig {
    fn default() -> Self {
        ...
        let app_paths = AppPaths::from_home(&home);
        Self {
            executable: discover_codex_executable(),
            supervisor_executable: std::env::current_exe().ok(),
            auth_path: codex_home.join("auth.json"),
            dashscope_env: app_paths.dashscope_env.clone(),
            temp_root: app_paths.runtime_directory.join("spark"),
...
impl SparkRunner {
    ...
    fn run_with_cancel_report(
        ...
        let mut command = if supervise {
            ...
            let dashscope_key = load_dashscope_env_secret(&self.config.dashscope_env)
                .map(|secret| String::from_utf8_lossy(secret.as_slice()).into_owned())
                .unwrap_or_default();
            spark_command(
                &self.config.executable,
                &run.codex_home,
    ...
    fn debug_dump(&self) {
        for base in [&self.root, &self.process_tmp] {
            ...
            if let Ok(entries) = fs::read_dir(base) {
                for entry in entries.flatten() {
                    Self::debug_dump_entry(entry.path(), 0);
                }
            }
        }
    }

    fn debug_dump_entry(path: PathBuf, depth: usize) {
        if depth > 4 {
            return;
        }
        if let Ok(md) = fs::symlink_metadata(&path) {
            let kind = if md.file_type().is_symlink() {
                format!(
                    "-> {:?}",
                    fs::read_link(&path).unwrap_or_default()
                )
            } else {
                String::new()
            };
            ...
            if md.is_dir() {
                if let Ok(entries) = fs::read_dir(&path) {
                    for entry in entries.flatten() {
                        Self::debug_dump_entry(entry.path(), depth + 1);
                    }
                }
            }
        }
...
fn audit_arg0_scratch(arg0: &Path, alias_target: &Path) -> Result<(usize, u64), SparkError> {
    ...
    for child in fs::read_dir(entry.path()).map_err(|_| SparkError::UnsafeWorkspace)? {
        ...
        if child_name == ".lock" {
            ...
            // Linux 版 Codex 会把这些别名指向「真实二进制所在的目录」,而不是二进制本身,
...
/// Linux 版 Codex 的 arg0 别名有两种形态：指向「真实二进制所在目录」，或直接指向
/// 二进制本身。两种都接受,但指向的 codex 必须是常规文件、属主可信、他人不可写、可执行。
fn is_trusted_codex_bin_dir(target: &Path) -> bool {
    let candidates = [target.join("codex"), target.to_path_buf()];
...
fn runtime_file_limit(name: &str) -> Option<u64> {
    match name {
        ...
        | "logs_2.sqlite"
        | "state_5.sqlite"
        | "queue_1.sqlite" => Some(MAX_RUNTIME_DATABASE_BYTES),
        "goals_1.sqlite-wal"
        | "memories_1.sqlite-wal"
        | "logs_2.sqlite-wal"
        | "state_5.sqlite-wal"
        | "queue_1.sqlite-wal" => Some(MAX_RUNTIME_WAL_BYTES),
        "goals_1.sqlite-shm"
        | "memories_1.sqlite-shm"
        | "logs_2.sqlite-shm"
        | "state_5.sqlite-shm"
        | "queue_1.sqlite-shm" => Some(MAX_RUNTIME_SHM_BYTES),
        // SQLite 在 rollback-journal 模式下会额外建 <-journal> 文件(新建的库默认就是
        // 这种模式),与 -wal 同级,给同样的上限。
        "goals_1.sqlite-journal"
        | "memories_1.sqlite-journal"
        | "logs_2.sqlite-journal"
        | "state_5.sqlite-journal"
        | "queue_1.sqlite-journal" => Some(MAX_RUNTIME_WAL_BYTES),
        _ => None,
    }
}
...
fn spark_command(
    ...
    command.args(SPARK_PROVIDER_CONFIG);
    // `--output-schema` 必须紧邻它的文件路径,否则新版 Codex 会认为缺少取值。
    ...
    command.env("QWEN_API_KEY", dashscope_key);
    inherit_proxy_environment(&mut command);
    command
}
...
"#,
    ...
    fn concurrent_runs_share_sweep_root_without_removing_live_workspaces() {
        ...
        let config_a = SparkRunnerConfig {
            ...
            dashscope_env: root.path().join(".env"),
