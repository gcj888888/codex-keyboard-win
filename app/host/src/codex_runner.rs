...
pub(crate) fn discover_codex_executable() -> PathBuf {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let mut candidates = entries
            ...
            .collect::<Vec<_>>();
        candidates.sort();
        return candidates.pop().unwrap_or_else(|| PathBuf::from("codex"));
    }
    #[cfg(target_os = "linux")]
    {
        let package = "codex-linux-x64";
        let vendor = "x86_64-unknown-linux-musl";
        let mut candidates: Vec<PathBuf> = Vec::new();
        let mut roots: Vec<PathBuf> = vec![home.join(".nvm/versions/node")];
        roots.extend(["/usr/local/lib/node_modules", "/usr/lib/node_modules"].map(PathBuf::from));
        for root in roots {
...
impl CodexRunner {
    ...
    pub fn run_with_cancel(&self, job: &Job, cancel: &AtomicBool) -> Result<(), JobFailureKind> {
        ...
        let mut command = if supervise {
            ...
            command.args(["exec", "resume", "--json", "--skip-git-repo-check", job.task_id.as_str(), "-"]);
            command
        };
        command
            .stdin(Stdio::piped())
...
pub fn run_supervisor(
    ...
    let mut child = match Command::new(executable)
        .args(["exec", "resume", "--json", "--skip-git-repo-check", task_id, "-"])
        .stdin(Stdio::inherit())
