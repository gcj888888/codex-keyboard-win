...
///
/// The Host may be started by a supervisor that spawns the binary directly,
/// outside any login session. Such a process inherits a minimal environment and
/// would miss `QWEN_API_KEY`, which makes every `codex exec` exit within
/// milliseconds. Reading the value here keeps the Host working regardless of how
/// it was launched. Note that `/etc/environment` is not a shell script and must
/// not be sourced: its default `PATH` entry is wrapped in quotes.
#[allow(unused_unsafe)]
#[cfg(target_os = "linux")]
fn load_environment_defaults() {
    ...
    for line in contents.lines() {
        ...
        if value.len() >= 2 {
            ...
            let quoted = (first == 34 && last == 34) || (first == 39 && last == 39);
            if quoted {
