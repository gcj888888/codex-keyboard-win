    ...
    match object.get("type").and_then(serde_json::Value::as_str) {
        Some("event_msg") => {
            ...
            match payload.get("type").and_then(serde_json::Value::as_str) {
                ...
                Some("task_complete") => {
                    let turn_id = required_turn_id(payload)?;
                    // Robustness: skip a completion whose turn was never opened in this
                    // scan window. This happens when a turn is superseded, or when the
                    // Host resumes scanning from the middle of a rollout file.
                    let Some(mut builder) = active.take() else {
                        return Ok(None);
                    };
                    if builder.turn_id != turn_id {
                        return Ok(None);
                    }
                    // Robustness: Codex can end a turn without producing a final answer,
                    // for example when it exits early on an internal error such as a
                    // missing environment variable or a failed sign-in. Such turns carry
                    // `last_agent_message: null` and hold nothing to summarize, so skip
                    // them while still advancing the scan position. Treating this as a
                    // malformed rollout used to wedge the observer permanently, which
                    // froze the running-task LED and the mailbox LEDs.
